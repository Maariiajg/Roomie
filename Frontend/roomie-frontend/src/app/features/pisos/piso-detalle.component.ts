      ...
      <div class="relative min-h-screen bg-black overflow-x-hidden font-sans">
        ...
        <div class="fixed bottom-8 inset-x-0 flex justify-center z-[200] px-6 gap-4">
          @if (isOwner()) {
            ...
            <button (click)="eliminarPisoAdmin()" class="bg-red-500 text-white px-12 py-5 rounded-full font-black uppercase tracking-widest text-sm shadow-[0_15px_30px_rgba(239,68,68,0.4)] hover:bg-red-600 active:scale-95 transition-all flex items-center gap-2">
...
export class PisoDetalleComponent implements OnInit, OnDestroy {
  ...
  isAdmin = computed(() => {
    return this.authService.role() === 'ADMINISTRADOR';
  });
  ...
  eliminarPisoAdmin() {
    const currentPiso = this.piso();
    if (!currentPiso) return;

    const confirmacion = confirm('⚠️ ¿Estás seguro de que deseas eliminar este piso de forma permanente? Todos sus datos y alquileres asociados se borrarán. Esta acción no se puede deshacer.');

    if (confirmacion) {
      this.pisoService.deletePiso(currentPiso.id).subscribe({
        next: () => {
          this.notificationService.showSuccess('Piso eliminado correctamente.');
