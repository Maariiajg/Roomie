  ...
  template: `
    <div class="bg-white rounded-[2rem] p-5 shadow-sm border border-gray-100 flex flex-col hover:shadow-xl hover:-translate-y-1 transition-all group">
      ...
      <div class="relative w-full h-48 mb-5 rounded-2xl overflow-hidden cursor-pointer" [routerLink]="['/piso', piso.id]">
        <img [src]="piso.fotos?.[0]?.url || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=600&q=80'" 
      ...
      <!-- Botón Favorito Absoluto sobre la tarjeta, pero fuera del routerLink de la imagen para que no navegue al pulsarlo -->
      <button (click)="toggleFavorito($event)" 
              class="absolute top-9 right-9 z-20 p-2.5 rounded-full bg-white/20 backdrop-blur-md hover:bg-white/40 hover:scale-110 transition-all shadow-md">
        <svg class="w-5 h-5 transition-colors" [ngClass]="isFavorito() ? 'text-red-500 fill-current' : 'text-white stroke-current fill-none'" viewBox="0 0 24 24" stroke-width="2.5">
      ...
      <div class="flex flex-col flex-grow">
        <a [routerLink]="['/piso', piso.id]" class="font-black text-lg text-textMain uppercase leading-tight hover:text-primary transition-colors line-clamp-1">
          {{ piso.direccion }}
        </a>
        <p class="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">Madrid</p>

        <!-- TIRA DE ICONOS UNIFICADA -->
        <div class="flex items-center gap-3 mt-4 mb-2">
          <div [ngClass]="piso.wifi ? 'text-primary' : 'text-gray-200'" title="WiFi">
          ...
          <div [ngClass]="piso.animales ? 'text-secondary' : 'text-gray-200'" title="Mascotas">
          ...
          <div [ngClass]="piso.garaje ? 'text-alert' : 'text-gray-200'" title="Garaje">
          ...
          <div [ngClass]="piso.tabaco ? 'text-orange-400' : 'text-green-500'" title="Tabaco">
        ...
        <div class="mt-auto border-t border-gray-100 pt-4 flex justify-between items-end">
          <div>
            ...
            <p class="text-xl font-black text-textMain leading-none">
              <!-- Calcula en Angular el precio por la cantidad actual + 1 para un nuevo residente -->
              {{ piso.precioMes / (piso.numOcupantesActual + 1) | currency:'EUR':'symbol':'1.0-0' }}<span class="text-xs text-gray-400 font-bold">/mes</span>
            </p>
          </div>
          <a [routerLink]="['/piso', piso.id]" class="p-3 bg-primary/10 text-primary rounded-xl hover:bg-primary hover:text-white transition-colors">
...
export class PisoCardComponent implements OnInit {
  ...
  toggleFavorito(event: Event) {
    event.stopPropagation(); // Evita navegar a la pagina de detalles si hace clic en el corazón
